...
(* function [[literal]], to infer the type of a literal constant ((prototype)) 438b *)
      fun literal value = 
          ...
          | NUM _ => (inttype, TRIVIAL)
          | BOOLV _ => (booltype, TRIVIAL)
          | NIL => raise TypeError "Is Nil"
          | PAIR (_,_)=> raise TypeError "Is Pair"
          | CLOSURE (_, _) => raise TypeError "CLOSURE"
          | PRIMITIVE _ => raise TypeError "Primitive"
...
(* function [[ty]], to infer the type of a \nml\ expression, given [[Gamma]] 438c *)
      fun ty (LITERAL n) = literal n
        ...
        | ty (IFX (e1, e2, e3))        = 
            let val (type_e1, con_e1) = ty e1
                val (type_e2, con_e2) = ty e2
                val (type_e3, con_e3) = ty e3
                val C1 = con_e1 /\ con_e2 /\ con_e3
                val C2 = (type_e1 ~ booltype) /\ (type_e2 ~ type_e3)
            in (type_e2, C1 /\ C2)
            end
        | ty (BEGIN es)                = 
          let 
        ...
        | ty (LAMBDA (formals, body))  = raise LeftAsExercise "t"
