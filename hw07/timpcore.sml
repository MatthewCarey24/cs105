...
(* function [[ty]], checks type of expression given $\itenvs$ ((prototype)) 348 *)
      | ty (AAT (a, i))        = 
            (case (ty a, ty i)
                ...
                |  _                    => raise TypeError 
                                        "array-at expects types (array t, int)"
           )
      | ty (APUT (a, i, e)) =
                (case (ty a, ty i, ty e) of
                    (ARRAYTY tau, INTTY, tau_e) =>
                        if eqType (tau_e, tau) 
                            then tau 
                        else
                            raise TypeError 
                                    "array-put expects types (array t, int, t)"
                | _ =>
                    raise TypeError "array-put expects types (array t, int, t)"
                 )
      | ty (AMAKE (len, init)) = 
                (case (ty len, ty init) of
                    (INTTY, tau_init) => ARRAYTY tau_init
                | _ => raise TypeError "array-make expects an int and a type"
                 )
      | ty (ASIZE a)           = 
                (case ty a of
                    (ARRAYTY tau) => INTTY
                | _ => raise TypeError "array-size expects ARRAYTY tau")
(* type declarations for consistency checking *)
