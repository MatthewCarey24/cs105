...
fun literal (BOOLV _)       = booltype
  | literal (NUM _)         = inttype
  | literal (SYM _)         = symtype
  | literal (PRIMITIVE _)   = raise InternalError 
                                "Attempted to type a primitive as literal"
  | literal (CLOSURE _)     = raise InternalError 
                                "Attempted to type a closure as literal"
  | literal (ARRAY _)       = raise InternalError 
                                "Attempted to type an array as literal"
  ...
  | literal (PAIR (x, xs))  = 
      ...
      in
        (case xs of
          NIL => listtype ty_x 
        | PAIR (y, ys) => 
            if eqType (ty_x, literal y) 
              then literal (PAIR (y, ys))
            else 
              raise TypeError "Ill typed list"
        | _ => raise TypeError "Expected a list or NIL")
      end
  ...
  let (* function [[ty]], checks type of expression given $\itenvs$ 338b *)
      fun ty (LITERAL v)                                 = literal v
        ...
        | ty (SET (x, e))                                = 
              let 
                val ty_e = ty e
                val ty_x = find (x, gamma)
              in 
                if eqType (ty_e, ty_x)
                  then ty_e
                else raise TypeError "type mismatch in set"
              end
        | ty (IFX (e1, e2, e3))                          = 
              (case (ty e1, ty e2, ty e3) of
                  (TYCON "bool", ty2, ty3) => 
                    if eqType (ty2, ty3)
                      then ty2
                    else 
                      raise TypeError "Branches of if must have same type"
                | _ => raise TypeError "1st expression of if must be bool"
                ) 
        | ty (WHILEX (e1, e2))                           = 
              (case (ty e1, ty e2) of
                  (TYCON "bool", ty2) => 
                    unittype
                | _ => raise TypeError "1st expression of while must be bool"
                    )
        | ty (BEGIN es)                                  = 
             let val exp_tys = map ty es
             in  List.last exp_tys handle Empty => unittype
             end                                                            
        | ty (APPLY (f, es))                             = 
              let 
                val actual_tys = map ty es
                val f_ty       = ty f
              in
                case f_ty of
                  FUNTY (param_tys, ret_ty) => 
                    if eqTypes (actual_tys, param_tys) 
                      then ret_ty
                    else raise TypeError 
                    "Type mismatch in func expectation and parameters passed"
                | _ => raise InternalError "Tried to apply non function"
              end
        | ty (LETX (LET, binds, exp))                    = 
             let
               val gam' = bindList (map fst binds, map (ty o snd) binds, gamma)
             in typeof (exp, delta, gam')
             end
        | ty (LETX (LETSTAR, (x,e)::binds, exp))         = 
              ty (LETX (LET, mkEnv (x::[], e::[]), LETX (LETSTAR, binds, exp)))
        | ty (LETX (LETSTAR, [],           exp))         = ty exp
        | ty (LETRECX (typed_binds, exp))                = 
            let
              val simp_tys = map (fn ((var, typ), expr) => 
                                        asType (typ, delta)) typed_binds
              val gamma'   = bindList 
                               (map (fst o fst) typed_binds, simp_tys, gamma)
              val exp_tys  = map (fn ((var, typ), expr) => 
                                       typeof(expr, delta, gamma')) typed_binds
              val exp_tau  = typeof (exp, delta, gamma')
            in 
              if eqTypes (simp_tys, exp_tys)
                then exp_tau
              else raise TypeError "Type mismatch in binds with expressions"
            end

        | ty (LAMBDA (params, lambda_exp))               = 
            let
              val simp_tys = map (fn (var, typ) => asType (typ, delta)) params
              val gamma'   = bindList (map fst params, simp_tys, gamma)
              val exp_tau  = typeof (lambda_exp, delta, gamma')
            in FUNTY (simp_tys, exp_tau)
            end
        | ty (TYLAMBDA (name_list, exp))                 =    
            let
              val gamma_fv = freetyvarsGamma gamma
              val _        = List.app 
                               (fn name => if List.exists (fn a => a = name) 
                                           gamma_fv
                                                then raise TypeError 
                                                                "var not free"
                                                else ()) name_list
              val delta' = foldl (fn (alpha, kind) => bind (alpha, TYPE, kind)) 
                           delta name_list
              val tau = typeof (exp, delta', gamma)
            in FORALL (name_list, tau)
            end
        | ty (TYAPPLY (exp, tyex_list))                  = 
            let
            (* ensure that all types have valid kinds *)
              val simple_tys = map (fn (typ) => asType (typ, delta)) tyex_list
        (* ensure that exp has polymorphic type and extract alphas and tau *)
              val exp_ty = ty exp
              val (alpha_list, tau) = 
                  (case exp_ty of
                      FORALL (alpha_list, tau) => (alpha_list, tau)
                    | _ => raise TypeError "Tyapply on non-polymorphic type")
            in tysubst (tau, ListPair.zip (alpha_list, tyex_list))
            end
...
fun typdef (def, delta, gamma) = 
  let
        fun tyd (VAL (name, exp)) =
              let
                val typ = typeof (exp, delta, gamma)
              in
                (bind (name, typ, gamma), typeString typ)
              end
          | tyd (VALREC (name, typ, exp))  = 
              let
                val kind = asType (typ, delta)
                val gamma' = bind (name, typ, gamma)
                val typ = typeof (exp, delta, gamma')
              in
                (gamma', typeString typ)
              end
        | tyd (EXP exp)                  = tyd (VAL ("it", exp))
        | tyd (DEFINE (name, ret_ty, (typed_params, body))) = 
            let
              val param_tys = map snd typed_params
            in
              tyd (VALREC (name, FUNTY (param_tys, ret_ty), 
                            LAMBDA (typed_params, body)))
            end
  in tyd def
  end
