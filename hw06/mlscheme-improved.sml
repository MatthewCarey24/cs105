...
(* freeIn : exp -> name -> bool; takes an expression and a name, and returns a 
   boolean of whether that name is free in the expression *)
fun freeIn e name = 
  let fun free (LITERAL v)                    = true
        | free (VAR n)                        = true
        | free (SET (n, exp))                 = (n = name) orelse free exp 
        | free (APPLY (func, args))           = 
                free func orelse (List.exists (fn arg => free arg) args)
        | free (LAMBDA (args, exp))           = free exp 
                                                andalso not (member name args)
        | free (BEGIN exps)                   = List.exists 
                                                    (fn exp => free exp) exps
        | free (IFX (e1, e2, e3))             = free e1 orelse free e2 
                                                                orelse free e3
        | free (WHILEX (e1, e2))              = free e1 orelse free e2
        | free (LETX (LET, binds, exp))       = (List.exists 
                                                  (fn bind => 
                                                     free (snd bind)) binds) 
                                                     orelse (free exp 
                                                             andalso not 
                                                               (member name 
                                                                 (List.map fst 
                                                                   binds)))
        | free (LETX (LETREC, binds, exp))    = not (member name 
                                                      (List.map fst binds)) 
                                                andalso 
                                                  ((List.exists 
                                                    (fn bind => 
                                                      free (snd bind)) binds) 
                                                    orelse (free exp))
        | free (LETX (LETSTAR, [], exp))      = free exp
        | free (LETX (LETSTAR, b :: bs, exp)) = free 
                                                (LETX (LET, [b], 
                                                    LETX (LETSTAR, bs, e)))
  in free e
  end
...
local
   ...
   val var_x = VAR "x"
   val lambda_y = LAMBDA (["y"], APPLY (VAR "equal?", [VAR "x", VAR "y"]))
   val lambda_x = LAMBDA (["x"], lambda_y)
 ...
 val () =
     ...
     (fn () => freeIn set_x1 "x")
 val () =
     Unit.checkAssert "freeIn (set undefined) true"
     (fn () => freeIn set_x1 "y")
 val () =
     Unit.checkAssert "freeIn var x true"
     (fn () => freeIn var_x "x")
 val () = 
     Unit.checkAssert "lambda_x true"
     (fn () => freeIn lambda_x "equal?")
 val () = 
     Unit.checkAssert "lambda_x false"
     (fn () => not (freeIn lambda_x "x"))
 val () = 
     Unit.checkAssert "lambda_y true1"
     (fn () => freeIn lambda_y "equal?")
 val () = 
     Unit.checkAssert "lambda_y true2"
     (fn () => freeIn lambda_y "x")
 val () = 
     Unit.checkAssert "lambda_y false"
     (fn () => not (freeIn lambda_y "y"))
 (* More unit tests go here ... *)
 end


(* improve takes the parameters of a LAMBDA exp and an environment, and returns
   the same parameters with an environment that only binds free variables *)
fun improve ((names, expr), envi) = ((names, expr), 
                                    List.filter 
                                      (fn (bind) => 
                                      freeIn (LAMBDA (names, expr)) (fst bind)) 
                                    envi)
